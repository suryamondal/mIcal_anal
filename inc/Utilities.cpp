...
{
  reals sum=0.,dx,dy,sumerr=0.;

    for (int i=0; i<data.n; i++)
    {
      // cout<<" data "<<i<<" "<<data.X[i]<<" "<<data.Y[i]<<" c "<<circle.a<<" "<<circle.b<<endl;
        dx = data.X[i] - circle.a;
        dy = data.Y[i] - circle.b;
        sum += SQR(sqrt(dx*dx+dy*dy) - circle.r)/data.err[i];
	sumerr+=1./data.err[i];
    }
    if(sumerr==0) {sumerr=1;}
    // return sqrt(sum/data.n);
    return sqrt(sum/sumerr);
}
