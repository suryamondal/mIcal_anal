...
SwimSwimmer::SwimSwimmer(double dist, double halfgap ) : //const VldContext& vldc) :
        ...
        fSPI(-1)
{
    paradef = DetectorParameterDef::AnPointer;//AAR:these variables added to include variable airgap
...
SwimSwimmer::SwimSwimmer(int pln, double dist, double halfgap) : //const VldContext& vldc) :
        ...
        fSPI(-1)
{
    //paradef = Ical0DetectorParameterDef::AnPointer;//AAR:these variables added to include variable airgap
    //fDistance = dist;
    //fHalfLayerThickness = halfgap;
...
{   
  ...
  //  <<"=============== "<< fIsForward <<" ====================="<<endl;
  //  <<"==============================================="<<endl;
  //  <<"getmom1 "<<particle.GetPosition().X()<<" "<<particle.GetPosition().Y()<<" "<<particle.GetPosition().Z()<<" "<<particle.GetMomentum().X()<<" "<<particle.GetMomentum().Y()<<" "<<particle.GetMomentum().Z()<<" "<<particle.GetMomentumModulus()<<" "
  //  <<particle.GetS()<<" "<<particle.GetRange()<<endl;


  //  double posz = particle.GetPosition().Z();
  ...
  for (int ij=0; ij<fNmaxStep; ++ij) {
    ...
    //    cout<<"SwimSwimmer getmom1 "<<xyz.X()<<" "<<xyz.Y()<<" "<<xyz.Z()<<" "<<particle.GetMomentum().X()<<" "<<particle.GetMomentum().Y()<<" "<<momZ<<" mommod "<<particle.GetMomentumModulus()<<" "<<direction[0]<<" "<< direction[1]<<" "<<direction[2]<<endl;
    ...
    if (particle.GetMomentumModulus()!=0.0) {
      ...
      /*
	if (fabs(localZ) < 0.001) {
	density = 0.00137; //RPC gas
	}	else if (fabs(localZ) <0.003) {
	density = 2.32;// quartz; 
	} else if (fabs(localZ) <0.004) {
	density = 1.09;// G10; 
	} else if (abs(localZ) < 0.0125) {
	density = 0.00001; //Foam (neglecting strips)
	} else {
	density = 7.847; //SwimGeo::GetSwimMaterialDensity(material); iron
	}
      */
      ...
      pos1[1] =pos[1]*1000;
      pos1[2] =pos[2]*1000;
      //cout<< fHalfLayerThickness <<" "<<fHalfAirGap<<endl;
      //if (abs(localZ) < fHalfLayerThickness-0.02){ // 125) {//AAR:these variables added to include variable airgap

      double dirTrack[3] = {0.0,0.0,1.0};
      double posTrack[3] = {100*pos[0],100*pos[1],100*pos[2]};
	...
	pFieldMap->ElectroMagneticField( pos1, Bx,By,BPlane);
	B[0] =Bx*1000;B[1]= By*1000; B[2]=0;
	// cout<<"pos[0] "<<pos[0]<<" "<<pos[1]<<" "<<pos[2]<<" "<<B[0]<<" "<<B[1]<<endl;
      } else {
	B[0] =0;B[1]=0; B[2]=0;
      }
      // cout<<"pos[0] "<<pos[0]<<" "<<pos[1]<<" "<<pos[2]<<" "<<B[0]<<" "<<B[1]<<" "<<icalGeometry->GetCurrentVolume()->GetName()<<endl;
	...
	fnbfield++;
      ...
      for (int i=0; i<3; i++) {dir[i] = mom[i]/mommag;/*cout <<mom[i];*/}
      //cout <<mommag<< endl;
      //Projected position;
      ...
      if (!fIsForward) { for (int i=0; i<3; i++) {dir[i] = -dir[i];};}

      //      if (fIsForward) {
      for (int i=0; i<3; i++) {projpos[i] = pos[i] + dist*dir[i];}
      //      } else {
      //	for (int i=0; i<3; i++) {projpos[i] = pos[i] - dist*dir[i];}
      //      }
	...
	if (fIsForward) {
      ...
      //      double planpos[3] = {0,0,pos[2]+fStep};
      //      double plandir[3] = {0,0,1};

      //      double newplanpos[3], newplandir[3], plane[6];
      //      anal_rotme(planpos,rot,newplanpos);
      //      anal_rotme(plandir,rot,newplandir);

      //      for (int i=0; i<3; i++) {
      //	plane[i] = newplanepos[i]; plane[i+3] = newplanedir[i];
      //      }
      double distep = 0; // total track length
      // Propagate track in "plane"
      //      trace_track_planef(buvz, track, particle.GetCharge(), plane, distep); //AAR: 1st argument added to pass the Field array.

      //      double charge = particle.GetCharge();
      //      if (fIsForward) { charge *=-1.;}
      //cout<<" 1 buvz "<< buvz[0]<<" "<< buvz[1] <<" "<<buvz[2];
	...
	if (fIsForward) {
      ...
      //      if ((posz>0.7&& posz<0.95) ||(posz >4.9 && posz<5.1)) cout<<"posmom3 "<<particle.GetCharge()<<" "<<newpos[0]<<" "<< newpos[1]<<" "<< newpos[2]<<" "<<newmom[0]<<" "<< newmom[1]<<" "<< newmom[2]<<" pos "<<pos[0]<<" "<< pos[1]<<" "<< pos[2]<<" "<<mom[0]<<" "<< mom[1]<<" "<< mom[2]<<endl;


      /*
	if (fIsForward) {
	xNext[0] =
	particle.GetPosition().X()+fStep*particle.GetDirection().X();
	xNext[1] = 
	particle.GetPosition().Y()+fStep*particle.GetDirection().Y();
	xNext[2] = 
	particle.GetPosition().Z()+fStep*particle.GetDirection().Z();
	} else {
	xNext[0] =
	particle.GetPosition().X()-fStep*particle.GetDirection().X();
	xNext[1] = 
	...
	tmpmom +=density*distep*elos; // 0.175;
      ...
      //      double tmpmom = mommag + (fIsForward) ? -density*distep*0.175 :  density*distep*0.175; //1.75 MeV/gm-cm^2
      //      cout <<"tmpmom "<<  tmpmom<<" "<< mommag <<" "<< int(fIsForward) <<" "<<density<<" "<<distep<<endl;

      for (int i=0; i<3; i++) {mom[i] = tmpmom*mom[i]/mommag;/*cout <<mom[i];*/}
      ...
      //      cout<<"posmom4 "<<ij<<" "<<pos[0]<<" "<< pos[1]<<" "<< pos[2]<<" "<<mom[0]<<" "<< mom[1]<<" "<< mom[2]<<" "<<fStep<<" "<<dir[2]<<" "<<direction[2]<<endl;
      //      cout <<"SwimSwimmer: start/end z " << startpos[2] <<  "/" << endl << " density " << density/1000. << " dzds " << " dRange  " << density*distep <<" dist "<< distToNextPlane<<" "<<fDistance<<endl;
      if (distToNextPlane >=fDistance) break;
    }

  }

  //  cout<<"getmom2 "<<particle.GetPosition().X()<<" "<<particle.GetPosition().Y()<<" "<<particle.GetPosition().Z()<<" "<<particle.GetMomentum().X()<<" "<<particle.GetMomentum().Y()<<" "<<particle.GetMomentum().Z()<<" "<<particle.GetMomentumModulus()<<" "
  //      <<particle.GetS()<<" "<<particle.GetRange()<<endl;
  //  return satisfied;
  //<< "------------------SwimSwimmer::Swim end----------------------"<<endl;
  fbave /= fnbfield; 
  //<<"\n"<<fbave<< " "<< fnbfield<<endl;
  return fbave;
}
...
{  
  ...
  for (int ij=0; ij<fNmaxStep; ++ij) {
    ...
    //     <<"direction "<<direction[0]<<" "<< direction[1]<<" "<<direction[2]<<" "<<xyz.x()<<" "<<xyz.y()<<" "<<posZ<<endl;
    ...
    //    if (fStep <fStepSize/10.) fStep = fStepSize/10.;
    ...
    if (particle.GetMomentumModulus()!=0.0) {
      ...
      // cout  <<"startpos is it in m  "<< startpos<<" "<<startmom <<endl;
  ...
  //  <<"getmom2 "<<particle.GetPosition().X()<<" "<<particle.GetPosition().Y()<<" "<<particle.GetPosition().Z()<<" "<<particle.GetMomentum().X()<<" "<<particle.GetMomentum().Y()<<" "<<particle.GetMomentum().Z()<<" "<<particle.GetMomentumModulus()<<" "<<particle.GetS()<<" "<<particle.GetRange()<<" "<<Plane<<" "<<nextplane<<endl;
  //  return satisfied;
...
void SwimSwimmer::anal_getnrot( double* n, double* rot ) {
    ...
    for (int i=0; i<9; i++) {rot[i] = initrot[i];}
...
void SwimSwimmer::anal_rotme(double* x,double* r,double* y) {
    ...
    for (int i=0; i<3; i++) {
        y[i]=0;
        for (int j=0; j<3; j++) {
            y[i] += r[j+3*i]*xt[j]; //r[j][i]*xt[j];
        }
    }
}

void SwimSwimmer::anal_rotmet(double* x, double* r,double* y) {
